...
export default function EditShop() {
  ...
  const [images, setImages] = useState([]);           // preview URLs (blob or original)
  const [imageFiles, setImageFiles] = useState([null, null, null]); // File objects or null
  ...
  const handleImageChange = (index, e) => {
    ...
    if (file) {
      ...
      // Track the actual File object
      const newFiles = [...imageFiles];
      newFiles[index] = file;
      setImageFiles(newFiles);

      setIsChangedImages(true); // at least one image changed
    }
  };
  ...
  const removeImage = (index) => {
    ...
    // Still changed only if another slot has a new File
    setIsChangedImages(newFiles.some(f => f !== null));
  };
  ...
  const handleSave = async () => {
    ...
    formData.append("isChanged", isChangedImages);

    if (isChangedImages) {
      // Only append slots that have a real new File; skip unchanged (null) slots
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const data = await res.json()
        console.log("data.shop is", data.shop)
        ...
        if (data.shop) {
          ...
          setSeconsaryPhoneNumber(data.shop.secondaryPhoneNumber)
          setWEblink(data.shop.weblink)
          setDescription(data.shop.description)
          const imageUrls = data.shop.images.map(image => image.url);
          setImages(imageUrls)
          setogimages(imageUrls)
          // imageFiles stays [null, null, null] — no new files yet
        } else {
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-xl shadow-blue-900/5 w-full max-w-[1300px] overflow-hidden border border-white">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50/30 p-6 md:p-10 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <h1 className="text-3xl md:text-5xl font-bold text-gray-900 mb-3 tracking-tight">Edit Details</h1>
              ...
              <div className="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-gray-100/80">
                      ...
                      <div key={index} className="relative group">
                        {images[index] ? (
                          <div className="h-40 w-full rounded-2xl border-2 border-gray-200 overflow-hidden relative shadow-sm">
                            ...
                            {/* Badge: shows if this slot has a new file or is the original */}
                            <span className={`absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded-full ${imageFiles[index] ? 'bg-green-500 text-white' : 'bg-gray-800/60 text-white'}`}>
                              {imageFiles[index] ? 'New' : 'Current'}
                            </span>
                            <button
                              type="button"
                ...
                <div className="mt-8 hidden lg:block">
                  <button
                    ...
                    type="button"
                    disabled={saving}
                    className="w-full py-4 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold rounded-2xl shadow-xl shadow-blue-600/20 transition-all text-lg flex items-center justify-center gap-2"
                  >
                    {saving ? (
                      <>
